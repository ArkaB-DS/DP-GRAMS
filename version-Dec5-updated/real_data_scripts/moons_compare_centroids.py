...
# Global settings
# ---------------------------------------------------------------------
results_dir = "results/moons_centroid_comparison"
os.makedirs(results_dir, exist_ok=True)

sns.set(style="whitegrid", context="talk")

# Base synthetic moons dataset for main clustering comparison
base_n_samples = 1000   
moons_noise = 0.15

X_raw, y_true = make_moons(
...
merge_n_clusters = len(np.unique(y_true))   # should be 2 for moons
...
# Hyperparameter/subsampling-effect settings for DP-GRAMS-C (moons)
...
print(
    f"\n[Moons, base n={base_n_samples}] Metrics "
...
# ---------------------------------------------------------------------
# Visualization of clusterings (2D moons, no PCA)
# ---------------------------------------------------------------------
X_2d = X
...
fig.suptitle(
    f"Clustering Comparison on Moons Dataset (base n = {base_n_samples})",
    fontsize=18,
    y=0.95,
)

save_path_clusters = os.path.join(
    results_dir, f"moons_clustering_comparison_base_n{base_n_samples}.png"
)
...
print(
    "\nAll moons privacy-utility experiments across n completed. "
...
for metric in metrics_names:
    ...
    fig.suptitle(
        f"Privacy-Utility ({metric}) on Moons\nDP-GRAMS-C vs DP-KMeans, 2x2 grid over n",
        y=0.98,
    )
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    out_path = os.path.join(
        results_dir, f"moons_privacy_utility_{metric.lower()}_grid_multi_n.png"
    )
...
def dpms_hparam_single_blobs(   # name kept for symmetry with earlier script
    n_samples,
    epsilon_modes,
    clip_multiplier,
    m,
    ...
    """
    Single DP-GRAMS-C run for subsampling-effect hyperparameter sweeps on moons.

    - Generate moons with n_samples.
    - Standardize features and compute true_means_scaled.
    - Run DP-GRAMS-C and compute ARI, NMI (in raw space) and MSE in
      standardized space.
    """
    # Generate synthetic moons for this config / run
    rs_data = base_seed + run
    X_raw_run, y_run = make_moons(
        n_samples=n_samples,
        noise=moons_noise,
...
def subsampling_effect_clip_grid_blobs(
    ...
    """
    Subsampling-effect for DP-GRAMS-C on moons:
    ...
    for n_sub in subsample_sizes:
        print(
            f"\n[Subsample-effect][C*][moons] n_sub={n_sub}, "
...
def subsampling_effect_m_grid_blobs(
    ...
    for n_sub in subsample_sizes:
        print(
            f"\n[Subsample-effect][m][moons] n_sub={n_sub}, "
...
print(
    ...
    f"experiments on moons across n, C^*, and m..."
)
print(
...
clip_txt_path = os.path.join(
    results_dir, "moons_dpgrams_c_subsample_effect_clip_stats.txt"
)
with open(clip_txt_path, "w") as f:
    for n_sub in n_samples_list_privacy:
        ...
        f.write("\n")
print(
    f"[saved] Moons: C^* subsampling-effect stats -> {clip_txt_path}"
)

# 2x2 grids for C^* subsampling-effect: ARI, NMI, MSE
clip_ari_grid_path = os.path.join(
    results_dir, "moons_dpgrams_c_subsample_effect_clip_ari_grid.pdf"
)
plot_subsample_effect_grid(
    ...
    y_label="ARI",
    title_prefix="DP-GRAMS-C on Moons: ARI vs $C^*$ across n",
    out_path=clip_ari_grid_path,
    log_x=True,
)

clip_nmi_grid_path = os.path.join(
    results_dir, "moons_dpgrams_c_subsample_effect_clip_nmi_grid.pdf"
)
plot_subsample_effect_grid(
    results_by_n=clip_results_by_n,
    subsample_sizes=n_samples_list_privacy,
    ...
    x_label=r"Clip Multiplier ($C^*$)",
    y_label="NMI",
    title_prefix="DP-GRAMS-C on Moons: NMI vs $C^*$ across n",
    out_path=clip_nmi_grid_path,
    log_x=True,
)

clip_mse_grid_path = os.path.join(
    results_dir, "moons_dpgrams_c_subsample_effect_clip_mse_grid.pdf"
)
plot_subsample_effect_grid(
    results_by_n=clip_results_by_n,
    subsample_sizes=n_samples_list_privacy,
    ...
    title_prefix="DP-GRAMS-C on Moons: MSE vs $C^*$ across n",
...
m_txt_path = os.path.join(
    results_dir, "moons_dpgrams_c_subsample_effect_minibatch_stats.txt"
)
with open(m_txt_path, "w") as f:
    for n_sub in n_samples_list_privacy:
        ...
        f.write("\n")
print(
    f"[saved] Moons: Minibatch subsampling-effect stats -> {m_txt_path}"
)

# 2x2 grids for m subsampling-effect: ARI, NMI, MSE
m_ari_grid_path = os.path.join(
    results_dir, "moons_dpgrams_c_subsample_effect_m_ari_grid.pdf"
)
plot_subsample_effect_grid(
    ...
    y_label="ARI",
    title_prefix="DP-GRAMS-C on Moons: ARI vs m across n",
    out_path=m_ari_grid_path,
    log_x=True,
)

m_nmi_grid_path = os.path.join(
    results_dir, "moons_dpgrams_c_subsample_effect_m_nmi_grid.pdf"
)
plot_subsample_effect_grid(
    results_by_n=m_results_by_n,
    subsample_sizes=n_samples_list_privacy,
    ...
    x_label="Minibatch size m",
    y_label="NMI",
    title_prefix="DP-GRAMS-C on Moons: NMI vs m across n",
    out_path=m_nmi_grid_path,
    log_x=True,
)

m_mse_grid_path = os.path.join(
    results_dir, "moons_dpgrams_c_subsample_effect_m_mse_grid.pdf"
)
plot_subsample_effect_grid(
    results_by_n=m_results_by_n,
    subsample_sizes=n_samples_list_privacy,
    ...
    x_label="Minibatch size m",
    y_label="Centroid MSE",
    title_prefix="DP-GRAMS-C on Moons: MSE vs m across n",
    out_path=m_mse_grid_path,
    log_x=True,
)

print(
    "\nAll moons experiments completed (base clustering, "
