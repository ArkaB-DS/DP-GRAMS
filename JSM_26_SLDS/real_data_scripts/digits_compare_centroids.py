...
ms_T = int(np.ceil(np.log(max(2, n))))   # MS iterations
ms_p = 0.1                     # seed fraction
delta = 1e-5
...
# *** FIXED: use keyword arguments so T is passed correctly ***
...
# ---------------------------------------------------------------------
eps_modes_list = [0.25, 0.5, 1.0, 2.5, 5]
...
def _run_dpms_single(
    ...
    """
    Single DP-GRAMS-C run for hyperparameter sweeps on Digits.
...
def sweep_clip_multiplier_digits(
    ...
    """
    Sweep over clip_multiplier (C^*) for DP-GRAMS-C on Digits.
...
def sweep_minibatch_digits(
    ...
    """
    Sweep over minibatch size m (as a fraction of n) for DP-GRAMS-C on Digits.
    n is fixed, so we return a single curve:
        - Centroid MSE vs m
        - ARI vs m
        - NMI vs m
...
plt.savefig(clip_plot_path_mse, dpi=300, bbox_inches="tight")
plt.show()
...
plt.savefig(clip_plot_path_ari, dpi=300, bbox_inches="tight")
plt.show()
...
plt.savefig(clip_plot_path_nmi, dpi=300, bbox_inches="tight")
plt.show()
...
plt.savefig(m_plot_path_mse, dpi=300, bbox_inches="tight")
plt.show()
...
plt.savefig(m_plot_path_ari, dpi=300, bbox_inches="tight")
plt.show()
...
plt.savefig(m_plot_path_nmi, dpi=300, bbox_inches="tight")
plt.show()
