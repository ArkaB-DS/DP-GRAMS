...
def dp_grams(
    ...
    # epsilon_iter = epsilon / (2 sqrt(2 T log(2/delta))).
    ...
    def compute_sigma(C):
        ...
        Compute Gaussian noise scale sigma using:
          ...
          - per-iteration epsilon_iter, delta_iter,
          - and the Gaussian mechanism calibration.

        This matches the sigma expression given in the privacy proof
        ...
        # Small-epsilon approximation: matches the simplified expression in the text.
        if epsilon <= 1:
    ...
    eta = 1.0 # step size; eta=1 keeps updates mean-shift-like.
    C = h ** (1 - d) * clip_multiplier
    # print("[DEBUG] C=",C)

    # Calibrate Gaussian noise using the theoretical sigma expression.
    sigma = compute_sigma(C)
    # print("[DEBUG] sigma=", sigma)
    # =======================================================
