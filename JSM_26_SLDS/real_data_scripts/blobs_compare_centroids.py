...
fig.suptitle(
    f"Clustering Comparison on Blobs Dataset",
    fontsize=18,
    y=0.95,
)
...
for metric in metrics_names:
    ...
    fig.suptitle(
        f"Privacy-Utility ({metric}) Tradeoff across n for Blobs Datasets",
        y=0.98,
    )
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    out_path = os.path.join(
        results_dir, f"blobs_privacy_utility_{metric.lower()}_grid_multi_n.pdf"
    )
