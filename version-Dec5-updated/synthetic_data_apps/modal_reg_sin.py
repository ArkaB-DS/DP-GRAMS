...
# Match structure of modal_reg_3.py (just different n-grid)
...
# Hyperparam sweep settings (analogous to modal_reg_3.py)
...
sigma_default = sigma_noise                     # noise level in generator
...
def sweep_clip_multiplier_modal(
    ...
    sinusoidal 2-component modal regression setting, across n in n_list.
    ...
    for idx, n in enumerate(n_list):
        ...
        X, Y, pms_ref, T_pms = make_sin_data_with_pms(n, sigma=sigma, seed=seed_n)
...
def plot_clip_multiplier_results_multi_n_modal(clip_results_by_n, clip_values, n_list, out_path):
    ...
    for sinusoidal 2-component modal regression.
    """
...
def plot_minibatch_results_grid_modal(m_results_by_n, n_list, out_path):
    ...
    (mirrors bivariate_4mix), sinusoidal 2-component modal regression.
    """
    ensure_dir(os.path.dirname(out_path))
    ...
    fig.suptitle("MSE vs m across n for Sinusoidal 2-Component DP Modal Regression", y=0.98)
...
def main():
    ...
    ax2.set_title("Differentially Private Modal Regression for Sinusoidal 2-Component Mixture")
    ax2.set_xlabel("X")
    ax2.set_ylabel("Y")
    ax2.set_xlim(0, 1)
    ...
    results = []
    stats_summary = "=== Privacy-Utility Experiments (DP-PMS vs PMS, Sinusoidal 2-Component Mixture) ===\n"
    ...
    for n in n_values:
        ...
        # PMS baseline at this n (now timed)
        T_pms = int(np.ceil(np.log(max(2, n))))
        t0_pms = time.perf_counter()
        Y_pms_est = partial_mean_shift(
            X_data, Y_data,
            mesh_points=None,
            bandwidth=None,
            T=T_pms
        )
        pms_runtime = time.perf_counter() - t0_pms
        pms_time_mean = float(pms_runtime)
        pms_time_std = 0.0  # single run; std = 0

        # Treat PMS estimate as reference modal regression curve
        pms_ref = np.column_stack([X_data, Y_pms_est])
        pms_mse = mode_matching_mse(pms_ref, pms_ref)  # = 0.0; for table consistency
        ...
        for eps in eps_values:
            ...
            msg = (
                ...
                f"PMS-time={pms_time_mean:.2f}±{pms_time_std:.2f}s, "
