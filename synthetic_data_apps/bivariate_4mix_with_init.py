...
# ---------------------------------------------------------------------
# Data generation: 4-corner Gaussian mixture
# ---------------------------------------------------------------------

def generate_4corners(n_samples, seed=None):
    """
    Generate a symmetric 4-component Gaussian mixture in 2D:
    centers at (+/-3, +/-3) with identity covariance.
    """
...
def make_data(n_samples, seed):
    ...
    data, true_modes = generate_4corners(n_samples, seed=seed)
...
def contour_plot_single(data, true_modes, ms_modes, dp_modes, results_dir):
    ...
    ax3d.set_title("Estimated KDE Surface for 4-Modal Bivariate Gaussian Mixture")
    if true_modes.size > 0:
    ...
    ax2d.set_title("Mode Estimation for 4-Modal Bivariate Gaussian Mixture")
...
# ---------------------------------------------------------------------
# DAP Initialization Visualization 
# ---------------------------------------------------------------------
...
def plot_dap_initialization(data, init_info, results_dir):
    ...
    with open(csv_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["x1", "x2", "prob"])
        for i in range(len(data)):
            writer.writerow([data[i, 0], data[i, 1], probs[i]])
    ...
    summary = (
        ...
        f"Prob min/max: {probs.min():.6g} / {probs.max():.6g}\n"
    )
...
def mse_vs_n_samples(n_samples_list, epsilon, delta, p, n_runs,
    ...
    for idx, n_samples in enumerate(n_samples_list):
        ...
        for run in range(n_runs):
            ...
            t0 = time.perf_counter()
            if run == 0:
                h_used, dp_raw, _ = dp_grams(
                    X=data, epsilon=epsilon, delta=delta,
                    T=None, h=h, p0=p, rng=rng,
...
def privacy_utility_plot(n_samples_list, epsilon_values,
    ...
    ax.set_ylabel("MSE")
    ax.set_title("Privacy-Utility Tradeoff for 4-Modal Bivariate Gaussian Mixture")
...
def plot_clip_multiplier_results_multi_n(clip_results_by_n, clip_grid, n_list, out_path):
    ...
    plt.ylabel("MSE")
    plt.title("MSE vs $C^*$ across n for 4-Modal Bivariate Gaussian Mixture")
...
def plot_minibatch_results_grid(m_results_by_n, n_list, out_path):
    ...
    fig.suptitle("MSE vs m across n for 4-Modal Bivariate Gaussian Mixture", y=0.98)
...
def main():
    ...
    for run in range(n_runs):
        ...
        if run == 0:
            h_used, dp_raw, dap_init_info = dp_grams(
                X=data, epsilon=epsilon, delta=delta,
