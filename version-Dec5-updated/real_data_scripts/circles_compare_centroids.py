...
# Global settings 
# ---------------------------------------------------------------------
results_dir = "results/circles_centroid_comparison"
os.makedirs(results_dir, exist_ok=True)

sns.set(style="whitegrid", context="talk")

# Base synthetic circles dataset for main clustering comparison
base_n_samples = 1000   
circles_noise = 0.1
circles_factor = 0.5
...
merge_n_clusters = len(np.unique(y_true))   # should be 2 for circles
ms_T = int(np.ceil(np.log(max(2, n))))      # iterations for mean-shift
ms_p = 0.1                                  # fraction of points used as seeds
delta = 1e-6                                
p_seed = 0.1                                # DP-GRAMS-C: p0 (seed fraction)
base_rng_seed = 41
n_runs = 20                                 # DP runs per epsilon, and for subsampling-effect
...
# Hyperparameter/subsampling-effect settings for DP-GRAMS-C (circles)
...
def relabel_clusters(y_true, y_pred, n_clusters=2):
    """
...
ms_merged_modes = merge_modes_agglomerative(
    raw_ms_modes, n_clusters=merge_n_clusters, random_state=base_rng_seed
)
...
# ---------------------------------------------------------------------
base_metrics_csv = os.path.join(
    results_dir, f"clustering_metrics_base_n{base_n_samples}.csv"
)
...
base_metrics_txt = os.path.join(
    results_dir, f"clustering_metrics_base_n{base_n_samples}.txt"
)
...
print(
    f"\n[Circles, base n={base_n_samples}] Metrics "
    "(raw 2D features for clustering, MSE on scaled features):"
)
print(
    f"{'Alg':<14} {'ARI':>6} {'NMI':>6} "
...
# ---------------------------------------------------------------------
# Visualization of clusterings (2D circles, no PCA)
# ---------------------------------------------------------------------
X_2d = X
...
fig.suptitle(
    f"Clustering Comparison on Circles Dataset (base n = {base_n_samples})",
    fontsize=18,
    y=0.95,
)

save_path_clusters = os.path.join(
    results_dir, f"circles_clustering_comparison_base_n{base_n_samples}.png"
)
...
# Storage:
dpms_metrics_multi = {
    n_samp: {m: [] for m in metrics_names} for n_samp in n_samples_list_privacy
}
dpkm_metrics_multi = {
    n_samp: {m: [] for m in metrics_names} for n_samp in n_samples_list_privacy
}
    ...
    for idx_n, n_samp in enumerate(n_samples_list_privacy):
        ...
        for eps_idx, eps in enumerate(eps_modes_list):
            ...
            for run in range(n_runs):
                ...
                labels_dpms_run = relabel_clusters(
                    yn, labels_dpms_run, merge_n_clusters
                )
                ...
                run_mse = mode_matching_mse(
                    true_means_scaled_n.copy(), modes_scaled_run.copy()
                )
            ...
            ari_list_dpms = np.array(ari_list_dpms, dtype=float)
            nmi_list_dpms = np.array(nmi_list_dpms, dtype=float)
            mse_list_dpms = np.array(mse_list_dpms, dtype=float)
            time_list_dpms = np.array(time_list_dpms, dtype=float)

            dpms_metrics_multi[n_samp]["ARI"].append(float(ari_list_dpms.mean()))
            dpms_metrics_multi[n_samp]["NMI"].append(float(nmi_list_dpms.mean()))
            dpms_metrics_multi[n_samp]["MSE"].append(float(mse_list_dpms.mean()))
            dpms_err_multi[n_samp]["ARI"].append(float(ari_list_dpms.std(ddof=1)))
            dpms_err_multi[n_samp]["NMI"].append(float(nmi_list_dpms.std(ddof=1)))
            dpms_err_multi[n_samp]["MSE"].append(float(mse_list_dpms.std(ddof=1)))
            dpms_time_mean_multi[n_samp].append(float(time_list_dpms.mean()))
            dpms_time_std_multi[n_samp].append(float(time_list_dpms.std(ddof=1)))
            ...
            writer.writerow(
                [
                    ...
                    eps,
                    "DP-GRAMS-C",
                    float(ari_list_dpms.mean()),
                    float(ari_list_dpms.std(ddof=1)),
                    float(nmi_list_dpms.mean()),
                    float(nmi_list_dpms.std(ddof=1)),
                    float(mse_list_dpms.mean()),
                    float(mse_list_dpms.std(ddof=1)),
                    float(time_list_dpms.mean()),
                    float(time_list_dpms.std(ddof=1)),
                ]
            )
            ...
            for run in range(n_runs):
                ...
                labels_dpkm_run = relabel_clusters(
                    yn, labels_dpkm_run, merge_n_clusters
                )
            ...
            ari_list_dpkm = np.array(ari_list_dpkm, dtype=float)
            nmi_list_dpkm = np.array(nmi_list_dpkm, dtype=float)
            mse_list_dpkm = np.array(mse_list_dpkm, dtype=float)
            time_list_dpkm = np.array(time_list_dpkm, dtype=float)

            dpkm_metrics_multi[n_samp]["ARI"].append(float(ari_list_dpkm.mean()))
            dpkm_metrics_multi[n_samp]["NMI"].append(float(nmi_list_dpkm.mean()))
            dpkm_metrics_multi[n_samp]["MSE"].append(float(mse_list_dpkm.mean()))
            dpkm_err_multi[n_samp]["ARI"].append(float(ari_list_dpkm.std(ddof=1)))
            dpkm_err_multi[n_samp]["NMI"].append(float(nmi_list_dpkm.std(ddof=1)))
            dpkm_err_multi[n_samp]["MSE"].append(float(mse_list_dpkm.std(ddof=1)))
            dpkm_time_mean_multi[n_samp].append(float(time_list_dpkm.mean()))
            dpkm_time_std_multi[n_samp].append(float(time_list_dpkm.std(ddof=1)))
            ...
            writer.writerow(
                [
                    ...
                    eps,
                    "DP-KMeans",
                    float(ari_list_dpkm.mean()),
                    float(ari_list_dpkm.std(ddof=1)),
                    float(nmi_list_dpkm.mean()),
                    float(nmi_list_dpkm.std(ddof=1)),
                    float(mse_list_dpkm.mean()),
                    float(mse_list_dpkm.std(ddof=1)),
                    float(time_list_dpkm.mean()),
                    float(time_list_dpkm.std(ddof=1)),
                ]
            )

print(
    "\nAll circles privacy-utility experiments across n completed. "
    f"Metrics saved to: {privacy_csv}"
)

# ---------------------------------------------------------------------
# Privacy-utility plots: ARI, NMI, MSE
# 2x2 grid (one subplot per n) for each metric.
# Within each subplot: both DP-GRAMS-C and DP-KMeans vs epsilon.
# ---------------------------------------------------------------------
...
for metric in metrics_names:
    ...
    fig.suptitle(
        f"Privacy-Utility ({metric}) on Circles\nDP-GRAMS-C vs DP-KMeans, 2x2 grid over n",
        y=0.98,
    )
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    out_path = os.path.join(
        results_dir, f"circles_privacy_utility_{metric.lower()}_grid_multi_n.png"
    )
...
def dpms_hparam_single_circles(
    n_samples,
    epsilon_modes,
    clip_multiplier,
    m,
    ...
    """
    Single DP-GRAMS-C run for subsampling-effect hyperparameter sweeps on circles.

    - Generate circles with n_samples.
    - Standardize features and compute true_means_scaled.
    - Run DP-GRAMS-C and compute ARI, NMI (in raw space) and MSE in
      standardized space.
    """
    # Generate synthetic circles for this config / run
    rs_data = base_seed + run
    X_raw_run, y_run = make_circles(
        n_samples=n_samples,
        noise=circles_noise,
        factor=circles_factor,
    ...
    # Standardize and compute true means for MSE
...
def subsampling_effect_clip_grid_circles(
    ...
    """
    Subsampling-effect for DP-GRAMS-C on circles:
    ARI / NMI / MSE vs C^* across subsample sizes.
    ...
    for n_sub in subsample_sizes:
        print(
            f"\n[Subsample-effect][C*][circles] n_sub={n_sub}, "
            f"epsilon_modes={epsilon_modes}"
        )
        stats_list = []
        for cm in clip_values:
            ari_vals, nmi_vals, mse_vals, time_vals = [], [], [], []
            for run in range(n_reps):
                ari, nmi, mse, rt = dpms_hparam_single_circles(
                    n_samples=n_sub,
                    epsilon_modes=epsilon_modes,
                    clip_multiplier=cm,
                    m=None,  # default minibatch inside dpms_private
                    run=run + int(1000 * cm),
                    base_seed=base_seed,
                )
                ari_vals.append(ari)
...
def subsampling_effect_m_grid_circles(
    subsample_sizes,
    m_frac_grid,
    epsilon_modes,
    ...
    """
    Subsampling-effect for DP-GRAMS-C on circles:
    ARI / NMI / MSE vs minibatch size m across subsample sizes.
    ...
    for n_sub in subsample_sizes:
        print(
            f"\n[Subsample-effect][m][circles] n_sub={n_sub}, "
            f"epsilon_modes={epsilon_modes}, C*={clip_multiplier_fixed}"
        )
        # Build m-grid as fractions of n_sub 
        m_grid = sorted(
            set(max(1, int(frac * n_sub)) for frac in m_frac_grid)
        )
        stats_list = []
...
print(
    "\n[Subsampling-effect] Starting DP-GRAMS-C subsampling-effect "
    f"experiments on circles across n, C^*, and m..."
)
print(
    f"Subsample sizes: {n_samples_list_privacy}, "
    f"runs per configuration: {n_runs_subsample_effect}"
)

# --- C^* sweep across subsample sizes ---
clip_results_by_n = subsampling_effect_clip_grid_circles(
...
clip_txt_path = os.path.join(
    results_dir, "circles_dpgrams_c_subsample_effect_clip_stats.txt"
)
with open(clip_txt_path, "w") as f:
    for n_sub in n_samples_list_privacy:
        ...
        f.write("\n")
print(
    f"[saved] Circles: C^* subsampling-effect stats -> {clip_txt_path}"
)

# 2x2 grids for C^* subsampling-effect: ARI, NMI, MSE
clip_ari_grid_path = os.path.join(
    results_dir, "circles_dpgrams_c_subsample_effect_clip_ari_grid.pdf"
)
plot_subsample_effect_grid(
    ...
    y_label="ARI",
    title_prefix="DP-GRAMS-C on Circles: ARI vs $C^*$ across n",
    out_path=clip_ari_grid_path,
    log_x=True,
)

clip_nmi_grid_path = os.path.join(
    results_dir, "circles_dpgrams_c_subsample_effect_clip_nmi_grid.pdf"
)
plot_subsample_effect_grid(
    results_by_n=clip_results_by_n,
    subsample_sizes=n_samples_list_privacy,
    ...
    x_label=r"Clip Multiplier ($C^*$)",
    y_label="NMI",
    title_prefix="DP-GRAMS-C on Circles: NMI vs $C^*$ across n",
    out_path=clip_nmi_grid_path,
    log_x=True,
)

clip_mse_grid_path = os.path.join(
    results_dir, "circles_dpgrams_c_subsample_effect_clip_mse_grid.pdf"
)
plot_subsample_effect_grid(
    results_by_n=clip_results_by_n,
    subsample_sizes=n_samples_list_privacy,
    ...
    title_prefix="DP-GRAMS-C on Circles: MSE vs $C^*$ across n",
    out_path=clip_mse_grid_path,
    log_x=True,
)

# --- m sweep across subsample sizes ---
m_results_by_n = subsampling_effect_m_grid_circles(
    subsample_sizes=n_samples_list_privacy,
    m_frac_grid=m_frac_grid_subsample,
    epsilon_modes=epsilon_subsample_effect,
    clip_multiplier_fixed=1.0,  # fixed C^*
    n_reps=n_runs_subsample_effect,
    base_seed=45678,
)
...
m_txt_path = os.path.join(
    results_dir, "circles_dpgrams_c_subsample_effect_minibatch_stats.txt"
)
with open(m_txt_path, "w") as f:
    for n_sub in n_samples_list_privacy:
        ...
        f.write("\n")
print(
    f"[saved] Circles: Minibatch subsampling-effect stats -> {m_txt_path}"
)

# 2x2 grids for m subsampling-effect: ARI, NMI, MSE
m_ari_grid_path = os.path.join(
    results_dir, "circles_dpgrams_c_subsample_effect_m_ari_grid.pdf"
)
plot_subsample_effect_grid(
    ...
    y_label="ARI",
    title_prefix="DP-GRAMS-C on Circles: ARI vs m across n",
    out_path=m_ari_grid_path,
    log_x=True,
)

m_nmi_grid_path = os.path.join(
    results_dir, "circles_dpgrams_c_subsample_effect_m_nmi_grid.pdf"
)
plot_subsample_effect_grid(
    results_by_n=m_results_by_n,
    subsample_sizes=n_samples_list_privacy,
    ...
    x_label="Minibatch size m",
    y_label="NMI",
    title_prefix="DP-GRAMS-C on Circles: NMI vs m across n",
    out_path=m_nmi_grid_path,
    log_x=True,
)

m_mse_grid_path = os.path.join(
    results_dir, "circles_dpgrams_c_subsample_effect_m_mse_grid.pdf"
)
plot_subsample_effect_grid(
    results_by_n=m_results_by_n,
    subsample_sizes=n_samples_list_privacy,
    ...
    x_label="Minibatch size m",
    y_label="Centroid MSE",
    title_prefix="DP-GRAMS-C on Circles: MSE vs m across n",
    out_path=m_mse_grid_path,
    log_x=True,
)

print(
    "\nAll circles experiments completed (base clustering, "
    "multi-n privacy-utility with 2x2 grids, and subsampling-effect). "
