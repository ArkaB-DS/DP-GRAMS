...
def plot_clip_multiplier_results_multi_n_modal(
    ...
    """
    Plot MSE vs C^* across n (single panel).
    """
...
def run_pms_repeated_for_configuration(
    ...
    Even though PMS is deterministic on fixed (X, Y),
    we still rerun it every time so that:
      - timing is measured across runs,
      - bookkeeping matches DP-PMS run structure,
      - PMS is not treated as a one-shot held-fixed baseline.

    Returns
    -------
    pms_refs_by_run : dict[int, np.ndarray]
        Maps run_idx -> PMS reference curve of shape (n, 2).
    pms_mses : list[float]
        Each is mode_matching_mse(pms_ref, pms_ref), so typically 0.
    ...
    # We create run-specific bookkeeping seeds even though PMS itself does not use them.
    ...
    for run_idx, seed in enumerate(pms_seeds):
        ...
        pms_ref_run = np.column_stack([X_copy, Y_pms_run])
        pms_mse_run = mode_matching_mse(pms_ref_run, pms_ref_run)

        pms_refs_by_run[run_idx] = pms_ref_run
    ...
    return pms_refs_by_run, pms_mses, pms_times
...
def run_dp_pms_repeated_for_configuration(
    ...
    comparing each run to the PMS reference from the same run index.
    ...
    with ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for fut in as_completed(futures):
            ...
            pms_ref_for_run = pms_refs_by_run[run_idx]
            est_modes = np.column_stack([mesh_ret, Y_refined])
            dp_mse = mode_matching_mse(pms_ref_for_run, est_modes)
...
def main():
    ...
    for n in n_values:
        ...
        for eps in eps_values:
            ...
            pms_refs_by_run, pms_mses, pms_times = run_pms_repeated_for_configuration(
                X_data=X_data,
                Y_data=Y_data,
                n=n,
                eps=eps,
            ...
            dp_mses, dp_times = run_dp_pms_repeated_for_configuration(
                ...
                n_runs_local=n_runs,
                delta=DELTA_DEFAULT,
                pms_refs_by_run=pms_refs_by_run,
            )
