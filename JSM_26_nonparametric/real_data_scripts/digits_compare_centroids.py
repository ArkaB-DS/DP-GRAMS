...
#
# Clustering comparison on the Digits dataset:
# - Non-private Mean Shift (MS) + merging
# - DP-GRAMS-C style: private modes + nearest-center labels (dpms_private)
# - Non-private KMeans
# - DP-KMeans (diffprivlib)
#
# Notes:
# - dpms_private uses (epsilon_modes, delta) for private modes.
# - Cluster labels are assigned deterministically via nearest private centroid
...
digits = load_digits()
X_raw = digits.data       # shape (n_samples, 64)
y_true = digits.target    # digits 0-9

n, d = X_raw.shape

merge_n_clusters = 10         # 10 digit classes
ms_T = int(np.log(n))        # MS iterations
ms_p = 1                     # all points as seeds
delta = 1e-5
p_seed = 1                   # DP-GRAMS-C seed fraction
base_rng_seed = 41
n_runs = 20

# We'll cluster in the original feature space,
# but compute centroid MSE in standardized space.
...
def timed(func, *args, **kwargs):
    """Return (result, runtime_seconds) for func(*args, **kwargs)."""
    start = time.time()
...
# ---------------------------------------------------------------------
# 1) Non-private Mean Shift baseline
# ---------------------------------------------------------------------
rng_ms = np.random.default_rng(base_rng_seed)
h_ms = silverman_bandwidth(X)  # heuristic bandwidth for MS
...
# ---------------------------------------------------------------------
# 2) DP-GRAMS-C style: dpms_private (private modes + nearest-centroid labels)
# ---------------------------------------------------------------------
...
try:
    ...
    # If bounds required, derive from data
    mins = X.min(axis=0)
    maxs = X.max(axis=0)
...
for alg, labels, modes in zip(algorithms, labels_list, modes_list):
    ...
    # Compare centroids in standardized feature space
...
print("\nMetrics (raw features; MSE on scaled centroids):")
print(f"{'Alg':<14} {'ARI':>6} {'NMI':>6} {'Silhouette':>10} "
      f"{'MSE_centroids':>14} {'Runtime(s)':>12}")
for alg, rt in zip(algorithms, runtimes):
    ari, nmi, sil = metrics[alg]
    print(
        f"{alg:<14} {ari:6.3f} {nmi:6.3f} {sil:10.3f} "
        f"{mse_centroids[alg]:14.6f} {rt:12.4f}"
    )

# ---------------------------------------------------------------------
# PCA visualization of cluster assignments + centroids
# ---------------------------------------------------------------------
...
for ax, alg, labels_pred, modes_2d in zip(axes, algorithms, labels_list, modes_2d_list):
    # Plot points by predicted cluster
    ...
    # Estimated centroids for this method
    modes_sc = ax.scatter(
        modes_2d[:, 0],
        modes_2d[:, 1],
        marker="X",
...
fig.suptitle("Clustering Comparison on Digits Dataset", fontsize=18, y=0.96)
...
# ---------------------------------------------------------------------
# Privacy–utility plots: 4 separate figures
# ---------------------------------------------------------------------
...
for metric in metrics_names:
    ...
    out_path = os.path.join(
        results_dir,
        f"digits_privacy_utility_{metric.lower()}.png"
    )
